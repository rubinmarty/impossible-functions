...
In fact, general tests for function equality are provably _impossible_ to perform, so it's no surprise that Python doesn't even try.
...
Now that we have our types, we can return to our motivation question, which will be thus: How can we write a function that will determine whether two Preds are equivalent to one another? If this seems like an impossible task, then read on.
...
In other words, `[True, False] + cantor` was no different than Python list addition normally is. However, we are only allowed to append to the left side of a Cantor (think about why).

Now that we have our definitions down, let's get to the hard part.

We define the following three functions:
  * `for_some` - determine whether there exists at least one possible input for which the predicate would return True
  * `for_every` - determine whether the predicate would return True on every single input
  * `find` - returns some Cantor which fulfils the input Pred. In the event that no such fulfilling Cantor exists, `find` can return any Cantor
...
`for_some` seems pretty straightforward (contingent on `find` functioning) - we just try to find a Cantor that satisfies our Pred, and if one exists then `for_some` returns True.

`for_every` is also pretty simple once we accept that `for_some` works - we just use a DeMorgan style piece of logic to say that a Pred holds on every single Cantor if its negation doesn't hold on any Cantors.
